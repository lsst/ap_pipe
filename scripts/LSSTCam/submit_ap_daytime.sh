...
nohup bps submit "${AP_PIPE_DIR}/bps/LSSTCam/bps_Daytime.yaml" \
  --extra-qgraph-options "--skip-existing-in LSSTCam/prompt/output-${DATE},LSSTCam/prompt/output-${DATE}/daytime -c parameters:release_id=1 -c parameters:apdb_config=${TMP_APDB} -c associateApdb:doRunForcedMeasurement=False --dataset-query-constraint off" \
